...
unsigned const int Erholungsresort::getGebaeudeAusfuhrungskosten() const
{
...
const std::stringstream Erholungsresort::getGebaudeAktivText() const
{
	...
	ssText << "Die Einheit "
	       << Daten->getEinheiten()[EinheitsVPosition].sName
...
void Erholungsresort::beendenDerAusfuhrung()
{
	...
	Daten->getKacheln(24).neueAnzeige(ssText.str(), 200, 99, 1,
					  1); // Akktualiesieren des Textes
	//// Hinzuf�gen aller Notiger Buttens
	Daten->getKacheln(24).ButtonHinzufuegen(
	    45, 450, 200, 30, 5, "Erholung starten", *Daten->getFont(),
	    sf::Color::Black, sf::Color(100, 100, 100), sf::Color(50, 50, 50),
	    sf::Color::White, Data->getKacheln(24).getGroese().x,
	    Data->getKacheln(24).getGroese().y);

	// Daten->getAnimationen().startBenarichtigung(true, "Die Einheit ist
	// vollstandig regeneriert");	TODO UI Animation
		...
		25) // �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		...
		if (Wirksamkeitsgrad > 26)
		{
			...
			ss << "Die Maximale Stufe\nwuerde erreicht.\nSie "
			      "koennen diesen\nPrarameter nicht "
			      "mehr\noprimieren";
			Daten->getKacheln(26).neueAnzeige(ss.str(), 350, 1, 535,
							  95);
		}
