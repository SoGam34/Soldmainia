...
{
	Daten->getKacheln(ProzessHauptKachel).TextAendern(
			getGebaudeAktivText().str(), 200);
}

void Gebaeude::beschleunigungDerAufgabenDurchfuehrung()
{
	if (Daten->getKontostand() > UpgradeKosten[0]
			&& AufgabenDurchfuehrungZeitFaktor >= 0.1)// �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		std::stringstream ss;
		ss << -UpgradeKosten[0];
		//Daten->getAnimationen().startBenarichtigung(false, ss.str());	TODO UI Animation
		...
		if (AufgabenDurchfuehrungZeitFaktor < 0.10)
		{
			...
			Daten->getKacheln(ProzessHauptKachel + 1).neueAnzeige(ss.str(), 350,
					1, 285, 95);
		}
		...
		else
		{
			...
			ss << "Beschleunigt die\nAusbildungsdauer um 5%\nKosten:"
					<< UpgradeKosten[0];
			Daten->getKacheln(ProzessHauptKachel + 1).TextAendern(ss.str(),
					350);
		}
	}
}
...
void Gebaeude::reduzierenDerAusfuhrungsKosten()
{
			...
			&& AusfuhrungsKostenFaktor > 10)// �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		...
		if (AusfuhrungsKostenFaktor == 10)
		{
			...
			ss
					<< "Die Maximale Stufe\nwuerde erreicht.\nSie koennen diesen\nPrarameter nicht mehr\noprimieren";
			Daten->getKacheln(ProzessHauptKachel + 3).neueAnzeige(ss.str(), 350,
					1, 785, 95);
		}
		...
		else
		{
			...
			Daten->getKacheln(ProzessHauptKachel + 3).TextAendern(ss.str(),
					350);
		}
	}
}
...
{
	VoraussichtlicheZeit = (Zeitversatz * AufgabenDurchfuehrungZeitFaktor
			* GebaeudeEinflussZeitFaktor * // Ermitteln der Zeit die Vorausichtlich f�r die Ausbildung gebraucht wird Unterberucksichtigung von der eines Faktors, der Gr��e, der Grundgeschwindikeit, der Bekanntheit
			((Daten->getBekanntheit() < 1000) ? 3 :
				(Daten->getBekanntheit() < 10000) ? 2 : 1)); // Ermitteln der Bekanntheit und dem dadurch resultierendem Faktor
