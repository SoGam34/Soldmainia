...
: 	Daten(data),
	ProzessAktiv(false),
	AufgabenDurchfuehrungZeitFaktor(1),
...
{
	if (Daten->getKontostand() > getGebaeudeAusfuhrungskosten()) // �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		std::stringstream ss;
		...
		Daten->setKontostand(Daten->getKontostand() - getGebaeudeAusfuhrungskosten());	// Abziehn der gesamten Ausbildungskosten
		ProzessAktiv = true;																			// Auf true gesetzt damit der Ausbildungsvortschrit angezietgt wird 
		Daten->getKacheln(ProzessHauptKachel).neueAnzeige(GebaudeAktivText().str(), 200, 99, 1, 1);	//neues Kachel Bild
	}
}

inline void Gebaeude::aktualisierenProzessZustand()
{
	Daten->getKacheln(ProzessHauptKachel).TextAendern(GebaudeAktivText().str(), 200);
}

void Gebaeude::BeschleunigungDerAufgabenDurchfuehrung()
{
	if (Daten->getKontostand() > UpgradeKosten[0] && AufgabenDurchfuehrungZeitFaktor >= 0.1)	// �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		std::stringstream ss;
		ss << -UpgradeKosten[0];
		//Daten->getAnimationen().startBenarichtigung(false, ss.str());	TODO UI Animation
		//Daten->getAnimationen().startUpgradeAnimation(2, Data->getBreite(), Data->getHohe());	TODO UI Animation

		Daten->setKontostand(Daten->getKontostand() - UpgradeKosten[0]);					// Abziehn der Verbesserungskosten
...
{
	if (Daten->getKontostand() > UpgradeKosten[2] && AusfuhrungsKostenFaktor > 10)	// �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		std::stringstream ss;
		ss << -UpgradeKosten[2];
		//Daten->getAnimationen().startBenarichtigung(false, ss.str());	TODO UI Animation
		//Daten->getAnimationen().startUpgradeAnimation(4, Data->getBreite(), Data->getHohe());	TODO UI Animation
		
		Daten->setKontostand(Daten->getKontostand() - UpgradeKosten[2]);			// Abziehn der Verbesserungskosten
		
		UpgradeKosten[2] *= 1.4;						// Speichern der neuen Verbesserungskosten
		AusfuhrungsKostenFaktor -= 10;

		ss.str("");
		if (!ProzessAktiv)	// �berpr�ft ob ein EM gesucht wird, wenn ja wird die Anzeige und  Uhr nicht aktualiesiert da dies zu Anzeigebugs f�hrt
		{
			BerrechnungVoraussichtlicheZeit();
			aktualisierenInformationsText();
		}
...
{
	VoraussichtlicheZeit = (Zeitversatz * AufgabenDurchfuehrungZeitFaktor * GebaeudeEinflussZeitFaktor * // Ermitteln der Zeit die Vorausichtlich f�r die Ausbildung gebraucht wird Unterberucksichtigung von der eines Faktors, der Gr��e, der Grundgeschwindikeit, der Bekanntheit
		((Daten->getBekanntheit() < 1000) ? 3 : (Daten->getBekanntheit() < 10000) ? 2 : 1));	 // Ermitteln der Bekanntheit und dem dadurch resultierendem Faktor
...
void Gebaeude::aktualisierenTimer()
{
	...
	if (getTimerstand() + Zeitversatz == 0 && ProzessAktiv)	// �berpr�fen ob die Zeit abgelaugen ist
		BeendenDerAusfuhrung();						// Beenden der Ausbildung, da die Ausbildung fertig ist 

	else if (ProzessAktiv)					// Aktualiesiern des Angezeigten Ausbildungs Fortschritts
