...
unsigned int Batillion_Ausbildungszentrum::getGebaeudeAusfuhrungskosten()
    const {
      return UpgradeStats.AusführungsReduzierungsKosten *
             (VoraussichtlicheZeit + Zeitversatz);
}

const std::stringstream Batillion_Ausbildungszentrum::getGebaudeAktivText()
    const {
...
void Batillion_Ausbildungszentrum::beendenDerAusfuhrung() {
      ...
      std::string name;
      std::cout << "Die Ausbildung des Battilion ist beendet wie wollen sie "
                   "das Battilion nennen? "
                << std::endl;
      std::cin >> name;

      Daten->addBattiliion(Battilion(
          Batillionsgroesse,
          Batillionsgroesse * 10 * UpgradeStats.GebaudeSpezielleFaktor,
          Batillionsgroesse * 10 * UpgradeStats.GebaudeSpezielleFaktor,
          Batillionsgroesse * 10,
          name));
