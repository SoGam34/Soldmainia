...
{ /*
	 ...
	 //cData->getKacheln(8).addTextfeld(sf::Color::Black, cData->getFont(),
     sf::Vector2f(40, 200)); Daten->getKacheln(8).ButtonHinzufuegen(35, 350,
     200, 30, 5, "Namen Bestehtigen", *Daten->getFont(), sf::Color::Black,
			 sf::Color(100, 100, 100), sf::Color(255, 150, 0),
     sf::Color::White, Data->getKacheln(8).getGroese().x,
	 ...
	 //Daten->getAnimationen().startBenarichtigung(true,"Ausbildung
     Batillion beendet");	TODO UI Animation*/
}
...
void Batillion_Ausbildungszentrum::vorbereiten_neueAusbildung()
{
	...
	/*Daten->getKacheln(8).neueAnzeige(ssText.str(), 200, 99, 1, 1);//
	Akktualiesieren des Textes
	// Hinzuf�gen aller Notiger Buttens
	Daten->getKacheln(8).ButtonHinzufuegen(35, 450, 200, 30, 1, "Starten",
			*Daten->getFont(), sf::Color::Black, sf::Color(100, 100,
	100), sf::Color(255, 150, 0), sf::Color::White,
	...
	Daten->getKacheln(8).ButtonHinzufuegen(35, 400, 200, 30, 12,
			"Weniger Mitglieder", *Daten->getFont(),
...
void Batillion_Ausbildungszentrum::erhohenDerGrundstarke()
{
		...
		25) // �berpr�fen ob die Ausbildung bezahlt werden kann
	{
		...
		ssk << -UpgradeKosten[1];
		// Daten->getAnimationen().startBenarichtigung(false,
		// ssk.str());	TODO UI Animation
		ssk.clear();
		...
		// Daten->setKontostand(Daten->getKontostand() -
		// UpgradeKosten[1]);			// Abziehn der
		// Verbesserungskosten fUpgradeKosten[1]= fUpgradeKosten[1] *
		// ((iLevel[1] < 9) ? cData->getUpgradeFaktorScoutbuero(2, 0) :
		// (iLevel[1] < 17) ? cData->getUpgradeFaktorScoutbuero(2, 1) :
		// cData->getUpgradeFaktorScoutbuero(2, 2)));
		// // Speichern der neuen Verbesserungskosten

		// Daten->getAnimationen().startUpgradeAnimation(3,
		// Daten->getBreite(), Daten->getHohe());	TODO UI
		// Animation
		...
		if (Grundstaerke > 24)
		{
			...
			ss << "Die Maximale Stufe\nwuerde erreicht.\nSie "
			      "koennen diesen\nPrarameter nicht "
			      "mehr\noprimieren";
			// Daten->getKacheln(10).neueAnzeige(ss.str(), 350, 1,
			// 535, 95);
		}
